...
function setRunner(root, fixOverrides = {}) {
  ...
  config.fix = { runner: './stub-runner.sh', filterFlag: '--filter', runScriptPrefix: ['run'], commands: ['lint:fix', 'format:fix'], ...fixOverrides };
...
test('L1: changed package → runner invoked with --filter <pkg> fix (per-target override)', () => {
  ...
  try {
    ...
    stubRunner(root);
    setRunner(root);
    appendFileSync(join(root, 'games/cityville/src/game.ts'), 'export const x = 9;\n');

    const r = runScript(root, SCRIPT, { input: '{}' });
    assert.equal(r.status, 0, r.stderr);
    const calls = readFileSync(join(root, 'runner-calls.txt'), 'utf8').trim().split('\n');
...
test('L2: failing fix → exit 2 with residue tail; stop_hook_active short-circuits', () => {
  ...
  try {
    ...
    setRunner(root);
    appendFileSync(join(root, 'apps/studio/src/main.ts'), 'export const y = 9;\n');
...
test('L3: single-package (no filter flag) uses run-prefix form; generated files ignored', () => {
  ...
  try {
    ...
    assert.equal(r.status, 0);
    assert.ok(!existsSync(join(root, 'runner-calls.txt')), 'generated-only change ran the fixer');
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

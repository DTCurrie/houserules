...
function installedWithCompactor() {
  ...
  assert.equal(
    runCli(['init', '--yes', '--modules=output-compactor', root]).status,
    0,
  );
  return root;
}
...
test('CP1: oversized Bash output → updatedToolOutput JSON + spill file + self-gitignore', () => {
  ...
  try {
    ...
    const updated = out.hookSpecificOutput.updatedToolOutput.stdout;
    assert.equal(out.hookSpecificOutput.hookEventName, 'PostToolUse');
    assert.ok(updated.startsWith('line 0:'), 'head preserved');
    assert.match(
      updated,
      /lines omitted — full output saved to \.claude\/tool-output\//,
    );
    ...
    assert.equal(
      readFileSync(join(spillDir, spills[0]), 'utf8'),
      big,

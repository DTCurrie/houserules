...
`agent-kit init` detects the repo, plans a set of declarative actions, previews them, and applies. Sixteen built-in modules cover hooks, skills, agents, rules, and reference docs. Ownership is explicit: kit-owned files are manifest-tracked and refreshed by `update`, seeds belong to you and are never overwritten, and a managed region means the kit writes only between its markers in a file you own.

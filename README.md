...
| `testing`          | off                               | `.claude/rules/testing.md`: colocate unit tests in `__test__` beside the code, prefer a unit over an end-to-end test for the same assertion, mock only I/O boundaries, behavioral `describe`/`it` names with no ID prefixes, and no comment explaining what an assertion checks. Path-scoped to test files, so it loads only when a test is in play    |
| `prose-voice`      | off                               | `.claude/rules/prose-voice.md`: plain sentences, no semicolons, em dashes rewritten away, filler cut, exact content byte-preserved. Covers markdown and the sentences inside code comments, so its `paths:` list mirrors `code-comments`. Path-scoped, so it loads whenever a matching file enters the working set, which a plain Read is enough to do |

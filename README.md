...
| `lint-fix`        | on when fix scripts found         | Stop hook: auto-fix changed packages, surface only unfixable residue (SubagentStop is wired but no-ops unless `fix.onSubagentStop` — parallel subagents would each fix every package at once)                                                        |
...
| `code-comments`   | off                               | `.claude/rules/code-comments.md`: no narration, comment only for divergence-from-convention or non-obvious domain logic, 200-char cap — path-scoped (`paths:` frontmatter), so it loads only when a matching source file is in play                  |
...
| `ready`           | off                               | `/ready` skill: off-context pre-handoff roll-up — one ready/not-ready verdict + the falsifiable acceptance checklist (pairs with `verify-changed` + `reviewers`)                                                                                     |
| `sweep`           | off                               | `/sweep` skill: shard a repo-wide mechanical edit into per-package writer subagents that report only counts — the orchestrator pays O(shards), never sees the match set                                                                              |

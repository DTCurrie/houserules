...
try {
  ...
  if (root) {
    ...
    if (counts) {
      ...
      const bits = [ahead ? `ahead ${ahead}` : null, behind ? `behind ${behind}` : null].filter(Boolean);
    ...
    if (changed.length) {
      ...
      for (const path of changed) {
        const t = targets.find((x) => (x.pathPrefix ? path.startsWith(x.pathPrefix) : true));
        if (t) byTarget.set(t.name, (byTarget.get(t.name) ?? 0) + 1);
      }
      if (changed.length > 25) {
        const summary = [...byTarget.entries()].map(([n, c]) => `${n} (${c})`).join(', ');
        lines.push(`[kit] uncommitted: ${changed.length} files${summary ? ` — ${summary}` : ''}`);
      } else {
        const shown = changed.slice(0, 10).join(', ');
        lines.push(`[kit] uncommitted (${changed.length}): ${shown}${changed.length > 10 ? ', …' : ''}`);
        if (byTarget.size) lines.push(`[kit] targets touched: ${[...byTarget.keys()].join(', ')}`);
      }
    }

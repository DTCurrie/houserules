...
function withDependents(seed, packages) {
...
function main() {
  ...
  try {
    if (!packages.length) {
      ...
      const byPath = (config.targets ?? [])
        ...
        .map((t) => ({ prefix: t.pathPrefix, name: t.packageName }))
      ...
      scope = [...all].map((name) => ({
        package: name,
        reason: changedNames.has(name) ? 'changed' : 'dependent',
      }));
    }
  } catch {
    degraded = true;
  ...
  if (degraded) {
    scope = packages.map((p) => ({ package: p.name, reason: 'full-scope' }));
  }

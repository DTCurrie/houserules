...
function stageInstalledScripts(): string {
  const root = mkdtempSync(join(tmpdir(), 'plugin-prose-install-'));
...
beforeAll(() => {
  dependencyFreeDir = mkdtempSync(join(tmpdir(), 'plugin-prose-bare-'));
...
describe('pr-description-lint.mjs invoked with no arguments', () => {
  it('exits 1 on an empty stdin body, reporting the fenced-wrapper finding', () => {
    const r = runStaged('pr-description-lint.mjs');
    expect(r.status).toBe(1);
    expect(r.stdout).toMatch(/\[pr-description\/fenced-wrapper\]/);
  });
...
describe('prose-lint.mjs invoked with no arguments', () => {
  it('exits 0 and reports no findings, since the file list is empty', () => {
    const r = runStaged('prose-lint.mjs');

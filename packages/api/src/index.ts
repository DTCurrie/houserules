...
/**
 * `KitConfig`/`KitConfigTarget` are contract, not internal, even though the kit's own
 * validator (`parseKitConfig`, `buildJsonSchema`, ...) is not: `Target` above IS
 * `KitConfigTarget`, and every plugin's `ctx.claude.kitConfig` is typed `KitConfig | null`.
 ...
 * config file are the installer's job and live in `@agent-kit/api/internal` instead.
 */
export type { KitConfig, KitConfigTarget } from './config.js';

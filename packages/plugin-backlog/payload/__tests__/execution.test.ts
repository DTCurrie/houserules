...
function stageInstalledScripts(): string {
  const root = mkdtempSync(join(tmpdir(), 'plugin-backlog-install-'));
...
beforeAll(() => {
  dependencyFreeDir = mkdtempSync(join(tmpdir(), 'plugin-backlog-bare-'));
...
describe('backlog-log.mjs invoked with no arguments in a non-git directory', () => {
  it('exits 0, reporting it needs a git work tree rather than crashing', () => {
    const r = runStaged('backlog-log.mjs');

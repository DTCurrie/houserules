#!/usr/bin/env node
/**
 * Assembles payload-dist/ for a plugin that ships one `.mts` hook script alongside prose.
 * `tsconfig.payload.json` compiles the script before this runs, so this step only copies the

...
`@houserules/cli` ships exactly 17 modules. Everything else lives in a plugin package.

| Module             | Default                   | What you get                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------ | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `core`             | always                    | shared libs, config-driven Bash guard (blocks `git commit/push/stash`, `git checkout/restore/reset --hard/clean`, `gh pr create`), `houserules.config.json`, read-only git permissions, CLAUDE.md seed (filled from detection) or staged additions                                                                                                                                                                                                    |
...
| `orchestrate`      | off                       | `/orchestrate` skill + `task-worker` agent (sonnet): drive a planned phase by slicing it on **file ownership**, writing the shared seam first, then dispatching one worker per slice in waves. You review reports, never diffs (pairs with `plans`). A module option adds variants of the worker (`task-worker-low`/`-high`/`-xhigh`, and `task-worker-research` with `WebFetch` and `WebSearch`), generated at install time from the one source file |
| `research`         | off                       | three off-context agents for planned phases: `research-spike` (one scoped question set, one cited report), `research-synth` (merge reports into one reference doc), `refactor-planner` (reconcile audit reports into a sliced refactor plan) (pairs with `plans`)                                                                                                                                                                                     |

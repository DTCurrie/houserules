...
function testingModule(api: PluginApi): ModuleDef {
  ...
  return {
    ...
    group: 'optional',
    hint(): string {
      return 'path-scoped rule: a test must fail when the behavior breaks, no coverage-chasing, colocate unit tests in `__tests__`, prefer units over end-to-end, behavioral test names, no comments explaining assertions';
    },
    ...
    plan(_ctx: Ctx, answers: Answers): Action[] {
      ...
      return [
        ...
        api.payload.script(
          id,
          'test-layout.mjs',
          'checks structural placement clauses of testing.md and testing-typescript/javascript.md: colocation in __tests__, __tests__ directory contents, one suffix convention, no .e2e.test tier, no test leaking into build output',
        ),
        api.payload.script(
          id,
          'test-config.mjs',

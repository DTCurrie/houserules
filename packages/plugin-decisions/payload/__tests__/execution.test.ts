...
function stageInstalledScripts(): string {
  const root = mkdtempSync(join(tmpdir(), 'plugin-decisions-install-'));
...
beforeAll(() => {
  dependencyFreeDir = mkdtempSync(join(tmpdir(), 'plugin-decisions-bare-'));
...
describe('decision-lint.mjs invoked with no arguments', () => {
  it('exits 0 and reports no findings, since the file list is empty', () => {
    const r = runStaged('decision-lint.mjs');
...
describe('decision-log.mjs invoked with no arguments in a non-git directory', () => {
  it('exits 0, reporting it needs a git work tree rather than crashing', () => {
    const r = runStaged('decision-log.mjs');
    expect(r.status).toBe(0);
    expect(r.stderr).toMatch(/requires a git work tree/);
  });
